...
/***/ function(module, exports, __webpack_require__) {
	...
	function bestRadius() {
	  return Math.max(100, Math.min(window.innerHeight - 50, window.innerWidth - labelWidth, 800) / 2);
	}
	...
	var offsetFunction = planetOffsets[0].fn;
	var showLines = true;

	var solarSystem = _d3Selection2.default.select("#solar-system");
	...
	var offsetControl = controls.append("div").selectAll("label").data(planetOffsets).enter().append("label").text(function (d) {
	  ...
	  offsetFunction = d.fn;
	  redraw(radius, _planetData.planets);
	});

	var toggleLines = controls.append("div").append("label").text("Show Orbit Lines").append("input").attr("type", "checkbox").property("checked", showLines).on("change", function (d, i) {
	  showLines = !showLines;
	...
	var arcs = g.append("g").classed("arcs", true).selectAll("circle.arc").data(_planetData.planets).enter().append("circle").classed("arc", true).attr("r", offsetFunction);

	// create the labels
	var labelSize = radius / _planetData.planets.length;
	var labels = g.append("g").classed("labels", true).selectAll("g.label").data(_planetData.planets).enter().append("g").classed("label", true).attr("transform", function (d, i) {
	  var y = i * labelSize;
	...
	// the starting line
	var meridian = g.append("line").classed("meridian", true).attr("x1", 0).attr("x2", 0).attr("y1", 0).attr("y2", -radius);

	var sun = g.append("circle").classed("sun", true).attr("r", 4);

	// create the planets
	var planetCircles = g.append("g").classed("planets", true).selectAll("circle.planet").data(_planetData.planets).enter().append("circle").classed("planet", true).attr("r", 3).attr("transform", function (d, i) {
	...
	function redraw(radius, planets) {
	  ...
	  var pixelLength = distancePerPixel(planets, radius - 25);
